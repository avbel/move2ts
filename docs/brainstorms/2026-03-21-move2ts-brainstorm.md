...
- If env var is also undefined, throw a dedicated error type (e.g., `InvalidConfigError`)

### Package ID Handling
- Generated file starts with a const reading from env variable
- If env var not set, throw `InvalidConfigError` at import/init time
...
### Error Handling
- Generate a shared `move2ts-errors.ts` module with `InvalidConfigError` class; each generated module imports from it
...
if (!marketplacePackageId) {
  throw new InvalidConfigError('MARKETPLACE_PACKAGE_ID environment variable is not set');
}
