...
import { InvalidConfigError, validateSuiAddress } from './move2ts-errors';
...
export class InvalidConfigError extends Error {
  override readonly name = 'InvalidConfigError' as const;
...
export function validateSuiAddress(value: string, name: string): string {
  if (!/^0x[0-9a-fA-F]{1,64}$/.test(value)) {
    throw new InvalidConfigError(`${name} is not a valid Sui address: ${value}`);
  }
  return value;
}
```
...
- [ ] Generate shared `move2ts-errors.ts` with `InvalidConfigError` and `validateSuiAddress`
